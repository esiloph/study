...
public class HufflepuffPlayer extends Player {
    ...
    public HufflepuffPlayer(String 이름, int 풀체, int 풀마, int 경험치, int 골드, int 공격력, int 방어력, int 마법공격력) {
        ...
        this.기숙사이름= "후플푸프";
        this.마법주문[0]=new MagicSpell("브래키움 엠멘도",(int)(Math.random()*마법공격력/2+풀마/10),10,"상대의 뼈가 없어져 치명적인 데미지를 입혔다.");
        this.마법주문[1]=new MagicSpell("스투페파이",(int)(Math.random()*마법공격력+풀마/5),20,"상대가 기절하고 치명적인 데미지를 입혔다");
        this.마법주문[2]=new MagicSpell("페트리피쿠스 토탈루스",(int)(Math.random()*5*마법공격력+풀마/5),(int) (풀마*0.5),"상대의 몸이 화석처럼 굳어져 데미지를 입혔다.");
