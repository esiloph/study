...
public class SlytherinPlayer extends Player {
    public SlytherinPlayer(String 이름, int 풀체, int 풀마, int 경험치, int 골드, int 공격력, int 방어력, int 마법공격력) {
        ...
        this.기숙사이름="슬리데린";
        this.마법주문[0]=new MagicSpell("인센디오",(int)(Math.random()*마법공격력/2+풀마/10),10,"엄청난 화력의 화염이 발생했다!");
        this.마법주문[1]=new MagicSpell("리덕토",(int)(Math.random()*마법공격력+풀마/10),30,"주변의 바위를 깨뜨려 상대에게 데미지를 줬다!");
        this.마법주문[2]=new MagicSpell("섹튬셈프라",(int)(Math.random()*5*마법공격력+풀마/3),(int) (풀마*0.7),"상대의 몸에 칼자국이 나며 치명적인 상처가 나타나 엄청난 괴로움을 느끼고 있다.");
        this.마법주문[3]=new MagicSpell("아바다 케다브다",99999,(int) (풀마*0.95),"지팡이에서 붉은 빛을 내며 주문이 시전되었다. " +
                "\n  상대의 영혼이 분리되며 사라진다.");
